...
public class ReadWrite {
	...
	static ArrayList<String> readFile(int row, int column) throws IOException {
		FileReader file_to_read = new FileReader("D:/maven2revision/parent-project/textfile/test.txt");
	...
	static void writeToFile(List<String> rowList, List<String> columnList, int row, int column) throws IOException {
		...
		BufferedWriter bw =  new BufferedWriter(new FileWriter("D:/maven2revision/parent-project/textfile/test.txt"));
		try {
			while(rowCtr < row) {
				columnCtr = 0;
